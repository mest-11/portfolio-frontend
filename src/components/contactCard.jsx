...
const ContactCard = () => {
  ...
  // Ensure the GitHub and LinkedIn links include the protocol
  const gitHubLink = user.userProfile?.gitHubLink
    ? `https://${user.userProfile.gitHubLink.replace(/^https?:\/\//, '')}`
    : '#';
  const linkedInLink = user.userProfile?.linkedInLink
    ? `https://${user.userProfile.linkedInLink.replace(/^https?:\/\//, '')}`
    : '#';
      ...
      <div className="w-full items-center justify-center flex">
        <ul className="flex flex-col items-start gap-2 has-[:last]:border-b-0 *:inline-flex *:gap-2 *:items-center *:justify-center *:border-b-[1.5px] *:border-b-stone-700 *:border-dotted *:text-xs *:font-semibold *:text-[#434955] pb-3">
          <li>
            ...
            <a href={`tel:${user.userProfile?.contact}`}>{user.userProfile?.contact}</a>
          </li>
          <li>
          ...
          <li>
            ...
            <a href={gitHubLink} target="_blank" rel="noopener noreferrer">GIT</a>
          </li>
        </ul>
      </div>
